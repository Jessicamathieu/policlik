...
import { MoreHorizontal, Camera, Edit3, Trash2 } from 'lucide-react';
    ...
    <Card className="transition-all duration-300">
      <CardHeader className="flex flex-row items-start justify-between p-4 pb-2">
        <div>
          ...
          <CardDescription>{format(parseISO(expense.date), "dd MMMM yyyy", { locale: fr })}</CardDescription>
        </div>
        <DropdownMenu>
      ...
      <CardContent className="p-4 pt-0 text-sm space-y-2">
         <div className="flex justify-between items-center">
          <span className="font-semibold text-lg">CAD${expense.amount.toFixed(2)}</span>
           <Badge variant={"outline"} className={cn("text-xs", statusBadgeColors[expense.status])}>
                {expense.status}
            </Badge>
        </div>
